...
Modified by Owen Kunhardt to add reinforcement learning
'''
...
class PlayerAIReinforce(BaseAI):
    def __init__(self, model, criterion, optimizer, device, memo_dic = {}):
    ...
    def getMove(self, grid):
        ...
        true_val, bestMove = self.iterative_deepening_expectimax(grid,1,4)
        state = self.getStateArray(grid, bestMove)

        true_val = np.array(true_val)
        state.to(self.device)
        true_val = torch.from_numpy(true_val).float().to(self.device)

        # train on instance (batch size of 1)
        output = self.model(state)
        
        loss = self.criterion(output, true_val)
    ...
    def getMoveLearning(self, grid): #uses shitton of memory
        #initialize time
        def log2(x):
            if x == 0:
                return 0
            return int(math.log(x,2))
        heuristic_values = [self.h1(grid), self.h2(grid), self.h4(grid), self.sum_of_tiles(grid), self.sum_of_squares(grid)]
        current_state = ''
        for row in grid.map:
            for val in row:
                current_state += str(log2(val)) + ','
        for val in heuristic_values:
            current_state += str(round(val, 5)) + ','
        self.outstr.append(current_state)
        self.timer = time.process_time()
    ...
    def getMoveLearningOld(self, grid):
        #initialize time
        def log2(x):
            if x == 0:
                return 0
        ...
        for row in grid.map:
            for val in row:
                self.outstr += str(log2(val)) + ','
        for val in heuristic_values:
            self.outstr += str(round(val, 5)) + ','
        self.outstr += '\n'
        self.timer = time.process_time()
    ...
    def iterative_deepening_expectimax(self, grid, start, end):
        ...
        for i in range(start, end):
            ...
            val, new_answer = self.expectimax(grid, i, True, -float('inf'), float('inf'))
            if not self.timed_out:
                answer = new_answer
                final_val = val
            else:
                break
        return final_val, answer
    ...
    def sum_of_squares(self, grid):
        return sum(sum(x**2 for x in row) for row in grid.map) / 1600
    ...
    def hnn(self, grid):
        ...
        for row in grid.map:
            for val in row:
                state.append(val)
        state += [self.h1(grid), self.h2(grid), self.h4(grid), self.sum_of_tiles(grid), self.sum_of_squares(grid)]
        state = np.array(state)
        state = state.reshape((21,1))
        state = state.T
        #print(state.shape)
    ...
    def heuristic(self, grid):
        ...
        state = self.getStateArray(grid, move)
...
# Marin
    ...
    def getStateArray(self, grid, bestMove):

        state = []
        for row in grid.map:
            for val in row:
                state.append(val)
        state += [self.h1(grid), self.h2(grid), self.h4(grid), self.sum_of_tiles(grid), self.sum_of_squares(grid), 0]
        state = np.array(state)
        state = state.reshape((22,1))
        state = state.T
        state = torch.from_numpy(state).float()
